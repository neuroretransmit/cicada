...
if __name__ == "__main__":
    import sys
    with open("107,167.jpg.runes.txt", "r") as runes, open("107,167.jpg.runes-possibilities.txt", "w") as firsttwo:
        data = runes.read()
        results = Gematria.rune_to_english(data, key=Gematria.key_to_shifts("FIRFUMFERENFE"), overrides={'ᚠ': 'F'}, fast=True)
        for result in results:
            firsttwo.write(result + "\n")
    with open("229.jpg.runes.txt", "r") as runes, open("229.jpg.runes-possibilities.txt", "w") as two29:
        data = runes.read()
